...
export async function createOrder(request, response) {
  ...
  const notes = String(request.body.notes || "")
    .trim()
    .slice(0, 1000);
  const customer = await Customer.findOne({ _id: request.body.customerId });
  if (!customer)
...
export async function confirmOrder(request, response) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      const order = await Order.findOne({ _id: request.params.id }).session(
        session,
      );
      if (!order)
    ...
    response.json(await populated(Order.findOne({ _id: saved })));
  } finally {
...
export async function rejectOrder(request, response) {
  ...
  response.json(await populated(Order.findOne({ _id: order.id })));
}
